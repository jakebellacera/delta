...
<Tabs>
  <TabItem label="SQL">
    ```sql ALTER TABLE grade SET TBLPROPERTIES ('delta.enableRowTracking' =
    'true'); ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="SQL">
    ```sql SET spark.databricks.delta.properties.defaults.enableRowTracking =
    true; ```
  </TabItem>
  <TabItem label="Python">
    ```python
    spark.conf.set("spark.databricks.delta.properties.defaults.enableRowTracking",
    True) ```
  </TabItem>
  <TabItem label="Scala">
    ```scala
...
<Tabs>
  <TabItem label="SQL">
    ```sql SELECT _metadata.row_id, _metadata.row_commit_version, * FROM
    table_name; ```
  </TabItem>
  <TabItem label="Python">
    ```python spark.read.table("table_name").select("_metadata.row_id",
    "_metadata.row_commit_version", "*") ```
  </TabItem>
  <TabItem label="Scala">
    ```scala spark.read.table("table_name").select("_metadata.row_id",
    "_metadata.row_commit_version", "*") ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="SQL">
    ```sql ALTER TABLE table_name SET TBLPROPERTIES (delta.enableRowTracking =
    false); ```
  </TabItem>
  <TabItem label="Python">
    ```python spark.sql("ALTER TABLE table_name SET TBLPROPERTIES
    (delta.enableRowTracking = false)") ```
  </TabItem>
  <TabItem label="Scala">
    ```scala spark.sql("ALTER TABLE table_name SET TBLPROPERTIES
    (delta.enableRowTracking = false)") ```
  </TabItem>
</Tabs>
...
- The row ids and row commit versions metadata fields cannot be accessed while reading the [Change data feed](/apache-spark-connector/change-data-feed/).

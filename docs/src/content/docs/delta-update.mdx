...
See the [Delta Lake APIs](/delta-lake-apis/) for details.
...
>
  ...
  vacuumed. See
  [vacuum](/apache-spark-connector/table-utility-commands/#remove-files-no-longer-referenced-by-a-delta-table)
  for details.
</Aside>
...
See [Configure SparkSession](/apache-spark-connector/table-batch-reads-and-writes/#configure-sparksession) for the steps to enable support for SQL commands.
...
See the [Delta Lake APIs](/delta-lake-apis/) for Scala, Java, and Python syntax details.
...
<Aside type="note">
  ...
  Java [Delta Lake APIs](/delta-lake-apis/) in Delta 2.3 and above. SQL is
...
- `whenMatched` clauses are executed when a source row matches a target table row based on the match condition. These clauses have the following semantics.
  ...
  - `whenMatched` clauses can have at most one `update` and one `delete` action. The `update` action in `merge` only updates the specified columns (similar to the `update` [operation](/apache-spark-connector/table-deletes-updates-and-merges/#update-a-table)) of the matched target row. The `delete` action deletes the matched row.
  ...
  - To update all the columns of the target Delta table with the corresponding columns of the source dataset, use `whenMatched(...).updateAll()`. This is equivalent to:
    ...
    > **Note**
    >
    > This behavior changes when automatic schema migration is enabled. See [Automatic schema evolution](/apache-spark-connector/table-deletes-updates-and-merges/#automatic-schema-evolution) for details.
...
<aside class="caution">
  ...
  example](/apache-spark-connector/table-deletes-updates-and-merges/#write-change-data-into-a-delta-table)—it
  shows how to preprocess the change dataset (that is, the source dataset) to
  retain only the latest change for each key before applying that change into
  the target Delta table. - You can apply a SQL `MERGE` operation on a SQL VIEW
  only if the view has been defined as `CREATE VIEW viewName AS SELECT * FROM
  deltaTable`.
</aside>
...
If you do not want the extra columns to be ignored and instead want to update the target table schema to include new columns, see [Automatic schema evolution](/apache-spark-connector/table-deletes-updates-and-merges/#automatic-schema-evolution).
...
- **Reduce the search space for matches**: By default, the `merge` operation searches the entire Delta table to find matches in the source table. One way to speed up `merge` is to reduce the search space by adding known constraints in the match condition. For example, suppose you have a table that is partitioned by `country` and `date` and you want to use `merge` to update information for the last day and a specific country. Adding the condition
  ...
  will make the query faster as it looks for matches only in the relevant partitions. Furthermore, it will also reduce the chances of conflicts with other concurrent operations. See [Concurrency control](/apache-spark-connector/concurrency-control/) for more details.

- **Compact files**: If the data is stored in many small files, reading the data to search for matches can become slow. You can compact small files into larger files to improve read throughput. See [Compact files](/apache-spark-connector/best-practices/#compact-files) for details.

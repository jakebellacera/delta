...
- The command runs with [automatic schema evolution](/apache-spark-connector/table-deletes-updates-and-merges/#automatic-schema-evolution) enabled
...
See [Drop Delta table features](/apache-spark-connector/drop-delta-table-features/) for more information on dropping Delta table features.

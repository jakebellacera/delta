...
>
  ...
  version](/apache-spark-connector/how-does-delta-lake-manage-feature-compatibility/#what-is-a-protocol-version).
  This protocol upgrade is irreversible. Tables with column mapping enabled can
  only be read in Delta Lake 1.2 and above.
</Aside>
...
For more examples, see [Rename columns](/apache-spark-connector/table-batch-reads-and-writes/#rename-columns).
...
[Drop columns](/apache-spark-connector/table-batch-reads-and-writes/#drop-columns).
...
- Enabling column mapping on tables might break downstream operations that rely
  ...
  [Change data feed limitations for tables with column mapping enabled](/apache-spark-connector/change-data-feed/#change-data-feed-limitations-for-tables-with-column-mapping-enabled).
...
- In Delta Lake 3.0 and above,
  ...
  [Tracking non-additive schema changes](/apache-spark-connector/table-streaming-reads-and-writes/#schema-tracking)

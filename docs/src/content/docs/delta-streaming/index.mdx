...
For many Delta Lake operations on tables, you enable integration with Apache Spark DataSourceV2 and Catalog APIs (since 3.0) by setting configurations when you create a new `SparkSession`. See [Configure SparkSession](/apache-spark-connector/table-batch-reads-and-writes/#configure-sparksession).
...
<Aside type="note">
  ...
  [configuration](/apache-spark-connector/table-batch-reads-and-writes/#data-retention)
  and the stream lags in processing, Delta Lake processes the data corresponding
  to the latest available transaction history of the source table but does not
  fail the stream. This can result in data being dropped.
</Aside>
...
The same `DataFrameWriter` options can be used to achieve the idempotent writes in non-Streaming job. For details [Idempotent writes](/apache-spark-connector/table-batch-reads-and-writes/#idempotent-writes).

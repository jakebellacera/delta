...
Some users in the community have reported that Snowflake, unlike [Trino](https://trino.io/docs/current/connector/delta-lake.html) or [Spark](/apache-spark-connector/table-batch-reads-and-writes/), is not using [Delta statistics](/apache-spark-connector/optimizations/#data-skipping) to do data skipping when reading Delta tables. Due to this bug, Snowflake may read a lot of unnecessary parquet files resulting in poor query performance and increased API call requests from cloud providers.
...
As a workaround, you can enable [Delta UniForm](/apache-spark-connector/universal-format-uniform/) to generate Iceberg metadata and read these tables as Iceberg tables from Snowflake.
...
See [Generate a manifest file](/apache-spark-connector/table-utility-commands/#generate-a-manifest-file) for details.
...
<Tabs>
  <TabItem label="SQL">
    ```sql CREATE OR REPLACE EXTERNAL TABLE my_parquet_data_table( id INT AS
    (VALUE:id::INT), part INT AS (VALUE:part::INT), ... parquet_filename VARCHAR
    AS split_part(metadata$filename, '/', -1) ) WITH LOCATION =
    @my_staged_table/ FILE_FORMAT = (TYPE = PARQUET) PATTERN =
    '.*[/]part-[^/]*[.]parquet' AUTO_REFRESH = true; ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="SQL">
    ```sql CREATE OR REPLACE EXTERNAL TABLE my_parquet_data_partitioned_table(
    id INT AS (VALUE:id::INT), part INT AS ( nullif( regexp_replace
    (metadata$filename, '.*part\\=(.*)\\/.*', '\\1'),
    '__HIVE_DEFAULT_PARTITION__' )::INT ), ... parquet_filename VARCHAR AS
    split_part(metadata$filename, '/', -1), ) WITH LOCATION =
    @my_staged_partitioned_table/ FILE_FORMAT = (TYPE = PARQUET) PATTERN =
    '.*[/]part-[^/]*[.]parquet' AUTO_REFRESH = true; ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="SQL">
    ```sql CREATE OR REPLACE VIEW my_delta_table AS SELECT id, part, ... FROM
    my_parquet_data_table WHERE parquet_filename IN (SELECT filename FROM
    delta-manifest-table); ```
  </TabItem>
</Tabs>

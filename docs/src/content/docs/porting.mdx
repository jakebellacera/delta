...
- **External readers**: Directly reading the data stored in Delta Lake. For information on how to read Delta tables, see [read a table](/apache-spark-connector/table-batch-reads-and-writes/#read-a-table).
...
<Tabs>
  <TabItem label="Python">
    ```python data = spark.read.format("parquet").load("/data-pipeline")
...
<Tabs>
  <TabItem label="Python">
    ```python spark.sql("CREATE TABLE events USING DELTA LOCATION
...
<Tabs>
  <TabItem label="SQL">
    ```sql CONVERT TO DELTA parquet.`/data-pipeline/` CREATE TABLE events USING
    DELTA LOCATION '/data-pipeline/' ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="SQL">
    ```sql CREATE TABLE events USING PARQUET OPTIONS (path '/data-pipeline/')
    CONVERT TO DELTA events ```
  </TabItem>
</Tabs>

For details, see [Convert a Parquet table to a Delta table](/apache-spark-connector/table-utility-commands/#convert-a-parquet-table-to-a-delta-table).
...
<Tabs>
  <TabItem label="Python">
    ```python spark.read \ .format("delta") \ .load("/the/delta/table/path") \
    .withColumnRenamed("column name", "column-name") \ .write \ .format("delta")
    \ .mode("overwrite") \ .option("overwriteSchema", "true") \
    .save("/the/delta/table/path") ```
  </TabItem>
  <TabItem label="Scala">
    ```scala spark.read \ .format("delta") \ .load("/the/delta/table/path") \
    .withColumnRenamed("column name", "column-name") \ .write \ .format("delta")
    \ .mode("overwrite") \ .option("overwriteSchema", "true") \
    .save("/the/delta/table/path") ```
  </TabItem>
...
If you are using `DeltaTable` APIs in Scala, Java, or Python to [update](/apache-spark-connector/table-deletes-updates-and-merges/) or [run utility operations](/apache-spark-connector/table-utility-commands/) on them, then you may have to add the following configurations when creating the `SparkSession` used to perform those operations.

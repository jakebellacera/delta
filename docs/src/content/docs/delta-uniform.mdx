...
- The table must have column mapping enabled. See [Delta column mapping](/apache-spark-connector/delta-column-mapping).
...
<Aside type="note">
  This syntax requires [Delta column
  mapping](/apache-spark-connector/delta-column-mapping) to be enabled on the
  table prior to running on Delta 3.1. This syntax also works to upgrade from
  the IcbergCompatV1. It may rewrite existing files to make those Iceberg
...
<Tabs>
  <TabItem label="Scala">
    ```scala spark.read.format("hudi") .option("hoodie.metadata.enable", "true")
...
- UniForm does not work on tables with deletion vectors enabled. See [What are deletion vectors?](/apache-spark-connector/what-are-deletion-vectors).

...
Delta Standalone provides classes and entities to read data, query metadata, and commit to the transaction log. A few of them are highlighted here and with their key interfaces. See the [Java API docs](/api/java/standalone/index.html) for the full set of classes and entities.

### DeltaLog

[DeltaLog](/api/java/io/delta/standalone/DeltaLog.html) is the main interface for programmatically interacting with the metadata in the transaction log of a Delta table.
...
A [Snapshot](/api/java/io/delta/standalone/Snapshot.html) represents the state of the table at a specific version.
...
[DeltaScan](/api/java/io/delta/standalone/DeltaScan.html) is a wrapper class for the files inside a `Snapshot` that match a given `readPredicate`.

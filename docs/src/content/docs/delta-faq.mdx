...
You can use your favorite Apache Spark APIs to read and write data with Delta Lake. See [Read a table](/apache-spark-connector/table-batch-reads-and-writes/#read-a-table) and [Write to a table](/apache-spark-connector/table-batch-reads-and-writes/#write-to-table).
...
Yes, you can use Structured Streaming to directly write data into Delta tables and read from Delta tables. See [Stream data into Delta tables](/apache-spark-connector/table-streaming-reads-and-writes/#delta-table-as-a-sink) and [Stream data from Delta tables](/apache-spark-connector/table-streaming-reads-and-writes/#delta-table-as-a-source).

## Does Delta Lake support writes or reads using the Spark Streaming DStream API?

Delta does not support the DStream API. We recommend [Table streaming reads and writes](/apache-spark-connector/table-streaming-reads-and-writes/).
...
Changing a column's type or dropping a column requires rewriting the table. For an example, see [Change column type](/apache-spark-connector/table-batch-reads-and-writes/#change-column-type-or-name).

...
description: Learn about Delta Lake's concurrency control mechanisms and how to handle write conflicts
---
...
- For supported [storage systems](/apache-spark-connector/storage-configuration), multiple writers across multiple clusters can simultaneously modify a table partition and see a consistent snapshot view of the table and there will be a serial order for these writes.
...
The following table describes which pairs of write operations can conflict. Compaction refers to [file compaction operation](/apache-spark-connector/best-practices/#compact-files) written with the option `dataChange` set to `false`.
...
<Tabs>
  <TabItem label="Scala">
    ```scala // Target 'deltaTable' is partitioned by date and country
    deltaTable.as("t").merge( source.as("s"), "s.user_id = t.user_id AND s.date
    = t.date AND s.country = t.country") .whenMatched().updateAll()

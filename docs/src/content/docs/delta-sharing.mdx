...
Delta Sharing data source supports most of the options provided by Apache Spark DataFrame for performing reads through [batch](/apache-spark-connector/table-batch-reads-and-writes), [streaming](/apache-spark-connector/table-streaming-reads-and-writes), or [table changes (CDF)](/apache-spark-connector/change-data-feed) APIs on shared tables. Delta Sharing doesn't support writing to a shared table. Please refer to the [Delta Sharing Repo](https://github.com/delta-io/delta-sharing/blob/main/README.md) for more details. Please follow the [quick start](https://github.com/delta-io/delta-sharing?tab=readme-ov-file#quick-start) to leverage the Delta Sharing python connector to discover the shared tables.

For Delta Sharing reads on shared tables with advanced Delta Lake features such as [Deletion Vectors](/apache-spark-connector/what-are-deletion-vectors) and [Column Mapping](/apache-spark-connector/delta-column-mapping), you need to enable integration with Apache Spark DataSourceV2 and Catalog APIs (since delta-sharing-spark 3.1) by setting the same configurations as Delta Lake when you create a new `SparkSession`. See [Configure SparkSession](/apache-spark-connector/table-batch-reads-and-writes/#configure-sparksession).
...
<Tabs>
  <TabItem label="SQL">
    ```sql SELECT * FROM mytable TIMESTAMP AS OF timestamp_expression SELECT *
    FROM mytable VERSION AS OF version ```
  </TabItem>
  <TabItem label="Python">
    ```python spark.read.format("deltaSharing").option("timestampAsOf",
    timestamp_string).load(tablePath)
    spark.read.format("deltaSharing").option("versionAsOf",
    version).load(tablePath) ```
  </TabItem>
  <TabItem label="Scala">
    ```scala spark.read.format("deltaSharing").option("timestampAsOf",
    timestamp_string).load(tablePath)
    spark.read.format("deltaSharing").option("versionAsOf",
    version).load(tablePath) ```
  </TabItem>
...
Delta Sharing Streaming supports the following functionalities in the same way as Delta Streaming: [Limit input rate](/apache-spark-connector/table-streaming-reads-and-writes/#limit-input-rate), [Ignore updates and deletes](/apache-spark-connector/table-streaming-reads-and-writes/#ignore-updates-and-deletes), [Specify initial position](/apache-spark-connector/table-streaming-reads-and-writes/#specify-initial-position)
...
Please remember to set the spark configurations mentioned in [Configure SparkSession](/apache-spark-connector/table-batch-reads-and-writes/#configure-sparksession) in order to read shared tables with Deletion Vectors and Column Mapping.

| Read Table Feature | Available since version |
| --- | --- |
| [Deletion Vectors](/apache-spark-connector/what-are-deletion-vectors) | 3.1.0 |
| [Column Mapping](/apache-spark-connector/delta-column-mapping) | 3.1.0 |
| [Timestamp without Timezone](https://spark.apache.org/docs/latest/sql-ref-datatypes.html) | 3.3.0 |
| [Type widening (Preview)](/apache-spark-connector/delta-type-widening) | 3.3.0 |

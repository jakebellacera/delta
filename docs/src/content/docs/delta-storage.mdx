...
For other languages and more examples of Delta table operations, see the [Quickstart](/apache-spark-connector/quickstart/) page.
...
<Tabs>
  <TabItem label="Bash">
    ```bash aws dynamodb create-table \ --region us-east-1 \ --table-name
    delta_log \ --attribute-definitions AttributeName=tablePath,AttributeType=S
    \ AttributeName=fileName,AttributeType=S \ --key-schema
    AttributeName=tablePath,KeyType=HASH \ AttributeName=fileName,KeyType=RANGE
...
<Tabs>
  <TabItem label="ini">
    ```ini spark.delta.logStore.s3.impl=io.delta.storage.S3DynamoDBLogStore ```
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="Bash">
    ```bash aws dynamodb update-time-to-live \ --region us-east-1 \ --table-name
    delta_log \ --time-to-live-specification "Enabled=true,
